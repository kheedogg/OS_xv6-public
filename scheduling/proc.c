...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->nice_value = 20;	// Default priority
  p->parent = p;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(struct proc *p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++) {
        if(p1->state != RUNNABLE)
          continue;
        if(highP->nice_value > p1->nice_value)	// larger value, lower priority
          highP = p1;
      }
      p = highP;
...
ps(int pid)
{
  ...
  for(p=ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if( ( p->pid == pid ) || ( pid==0 ) ) {
      ...
      else if( ( p->state == RUNNING ) || ( p->state == RUNNABLE ) )
        cprintf("%d \t %d \t %d \t run \t %s \n", p->pid, p->parent->pid, p->nice_value, p->name);
